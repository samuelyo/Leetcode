...
public class Minesweeper {
	...
	public static char[][] updateBoard(char[][] board, int[] click) {
		...
		return updateBoardHelp(board, click, visit);
	}

	public static char[][] updateBoardHelp(char[][] board, int[] click, boolean[][] visit){
		
		int m = click[0];
		int n = click[1];
		
		if(m < 0 || m >= board.length || n < 0 || n >= board[0].length || visit[m][n] == true){
			return board;
		}
		...
		if(count != 0){
			...
			board[m][n] = 'B';
			updateBoardHelp(board, new int[]{m-1,n-1}, visit);
			updateBoardHelp(board, new int[]{m-1,n}, visit);
			updateBoardHelp(board, new int[]{m-1,n+1}, visit);
			updateBoardHelp(board, new int[]{m,n-1}, visit);
			updateBoardHelp(board, new int[]{m,n+1}, visit);
			updateBoardHelp(board, new int[]{m+1,n-1}, visit);
			updateBoardHelp(board, new int[]{m+1,n}, visit);
			updateBoardHelp(board, new int[]{m+1,n+1}, visit);
