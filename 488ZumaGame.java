...
public class ZumaGame {
	...
	static int MAXCOUNT = 6; // the max balls you need will not exceed 6 since
								// "The number of balls in your hand won't
								// exceed 5"
	...
	public static int findMinStep(String board, String hand) {
		...
		int rs = helper(board + "#", handCount); // append a "#" to avoid
													// special process while
													// j==board.length, make the
													// code shorter.
	...
	private static int helper(String s, int[] h) {
		...
		for (int i = 0, j = 0; j < s.length(); ++j) {
			if (s.charAt(j) == s.charAt(i))
				continue;
			need = 3 - (j - i); // balls need to remove current consecutive
								// balls.
