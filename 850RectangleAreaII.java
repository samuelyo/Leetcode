package com.leetcode.RectangleAreaII;
...
public class RectangleAreaII {
	...
	// Add new rectangle to the list. In case of overlap break up new rectangle
	// into non-overlapping rectangles. Compare the new rectanlges with the rest of
	// the list.
	public static void addRectangle(List<int[]> recList, int[] curRec, int start) {
		...
		if (curRec[1] < r[1])
			addRectangle(recList, new int[] { Math.max(r[0], curRec[0]), curRec[1], Math.min(r[2], curRec[2]), r[1] },
					start + 1);

		if (curRec[3] > r[3])
			addRectangle(recList, new int[] { Math.max(r[0], curRec[0]), r[3], Math.min(r[2], curRec[2]), curRec[3] },
					start + 1);
	}
